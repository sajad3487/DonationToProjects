                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap py-5">
                        ...
                        <div class="card-toolbar">
                            <a href="" data-toggle="modal" data-target="#new_solution" class="btn btn-light-success font-weight-bold mr-2">New Solution Providers</a>
                        </div>
                    </div>
...
{{--                                    <th>Profile Pic</th>--}}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="" data-toggle="modal" data-target="#edit_solution_{{$solution->id ?? ''}}">
                                            <i class="flaticon-edit text-warning mr-5"></i>
                                        </a>
                                        <div class="modal fade" id="edit_solution_{{$solution->id ?? ''}}" role="dialog"  aria-hidden="true">
                                            <div class="modal-dialog modal-xl" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title">{{$solution->title ?? ''}}</h5>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                            <i aria-hidden="true" class="ki ki-close"></i>
                                                        </button>
                                                    </div>
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="card-body text-left p-0">
                                                            ...
                                                            <form  action="{{url("/admin/solutions/$solution->id/update")}}" method="post" enctype="multipart/form-data">
                                                                @csrf
                                                                @method('PUT')
                                                                <div class="card-body p-3">
                                                                    @include('fragment.error')
                                                                        ...
                                                                        <div class="col-12 col-md-6">
                                                                            <div class="form-group mt-1">
                                                                                ...
                                                                                <select class="form-control" id="exampleSelect1" name="category_id" required>
                                                                                    <option>Select a category for the solution</option>
                                                                                    @foreach($categories as $category)
                                                                                        <option @if($category->id == $solution->category_id) selected @endif value="{{$category->id ?? ''}}">{{$category->title ?? ''}}</option>
                                                                                        @foreach($category->activeSubCat as $subCategory)
                                                                                            <option @if($subCategory->id == $solution->category_id) selected @endif value="{{$subCategory->id ?? ''}}"> ---- {{'  '.$subCategory->title ?? ''}}</option>
                                                                                        @endforeach
                                                                                    @endforeach
                                                                                </select>
                                                                            </div>
                                                                        ...
                                                                        <div class="col-12 col-md-6">
                                                                            <div class="form-group mt-1">
                                                                                ...
                                                                                <select class="form-control" id="exampleSelect" name="user_id" required>
                                                                                    <option>Select a user to assignee</option>
                                                                                    @foreach($owners as $owner)
                                                                                        <option @if($owner->id == $solution->user_id) selected @endif value="{{$owner->id ?? ''}}">{{$owner->name ?? ''}}</option>
                                                                                    @endforeach
                                                                                </select>
                                                                            </div>
                                                                        </div>
                                                                    ...
                                                                    <div class="form-group mt-1">
                                                                        <label for="exampleInputPassword1">Brief description:</label>
                                                                        <input type="text" name="brief_description" class="form-control"  value="{{$solution->brief_description ?? ''}}" id="exampleInputPassword1" placeholder="Enter the brief description of the solution"/>
                                                                    </div>
                                                                    <div class="form-group mt-1 mb-1">
                                                                        <label for="exampleTextarea">Description:</label>
                                                                        <textarea class="form-control" id="exampleTextarea"  name="description" rows="2">{{$solution->description ?? ''}}</textarea>
                                                                    </div>
                                                                    <div class="form-group mt-1 mb-1">
                                                                        <label for="exampleTextarea1">Issue:</label>
                                                                        <textarea class="form-control" id="exampleTextarea1" name="issue" rows="2">{{$solution->issue ?? ''}}</textarea>
                                                                    </div>
                                                                    <div class="form-group mt-1 mb-1">
                                                                        <label for="exampleTextarea2">Solution:</label>
                                                                        <textarea class="form-control" id="exampleTextarea2" name="solution" rows="2">{{$solution->solution ?? ''}}</textarea>
                                                                    </div>
                                                                    <div class="form-group mt-1 mb-1">
                                                                        <label for="exampleTextarea3">The Function:</label>
                                                                        <textarea class="form-control" id="exampleTextarea3" name="our_function" rows="2">{{$solution->our_function ?? ''}}</textarea>
                                                                    </div>
                                                                    <div class="form-group mt-1 mb-1">
                                                                        <label for="exampleTextarea4">Reason:</label>
                                                                        <textarea class="form-control"  id="exampleTextarea4" name="reason" rows="2">{{$solution->reason ?? ''}}</textarea>
                                                                    </div>
                                                                    <div class="form-group mt-1 mb-1">
                                                                        <label for="exampleTextarea5">Effect:</label>
                                                                        <textarea class="form-control" id="exampleTextarea5" name="effect" rows="2">{{$solution->effect ?? ''}}</textarea>
                                                                    </div>
                                                                    <div class="form-group mt-1">
                                                                        <label for="address">Address:</label>
                                                                        <input type="text" name="address" class="form-control" value="{{$solution->address ?? ''}}" id="address" placeholder="Enter the address"/>
                                                                    </div>
